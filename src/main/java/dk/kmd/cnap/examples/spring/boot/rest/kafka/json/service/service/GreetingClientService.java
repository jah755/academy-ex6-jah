...
public class GreetingClientService {
  ...
  // For convinience, and so that our /api/response endpoint works,
  // we store in-memory, the final response, GreetingCreateResponse (coming from the TOPIC_GREETING_CREATE_RESPONSE)
  ...
  //  (topics -> what topics to listen on, containerFactory -> defines which ConcurrentKafkaListenerContainerFactory exactly should be used, groupId -> defines consumer groupId  )
            ...
            (recordMetadata, e) -> {
              if (e != null)
                logBuilder()
                        ...
                        .parameter("kafkaTopic", KafkaTopicName.TOPIC_GREETING_CREATE_REQUEST)
